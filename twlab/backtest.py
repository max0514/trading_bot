...
change. Like FinLab, returns are simulated on Adjusted Prices (`etl:adj_*`)
so holding across an ex-dividend date books no phantom crash; if the `etl`
Dataset is not materialized yet it falls back to raw prices with a warning.
It returns the engine's Report (equity curve, positions, trades, metrics).
"""
...
_ADJUSTED = {"close": ("etl:adj_close", "price:收盤價"), "open": ("etl:adj_open", "price:開盤價")}
...
def _prices(kind: str) -> pd.DataFrame:
    adjusted, raw = _ADJUSTED[kind]
    try:
        return pd.DataFrame(data.get(adjusted))
    except DatasetNotMaterializedError:
        warnings.warn(
            f"{adjusted} is not materialized; simulating on raw {raw} — returns across "
            f"除權息 dates will show phantom moves until `python -m twlab.pipeline etl` runs",
            UserWarning, stacklevel=3,
        )
        return pd.DataFrame(data.get(raw))
...
def sim(position: pd.DataFrame, resample: str = "M", *,
    ...
    close = pd.DataFrame(close_price) if close_price is not None else _prices("close")
    open_ = pd.DataFrame(open_price) if open_price is not None else _prices("open")
